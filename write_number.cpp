...
{   
    DWORD t1,PICTURE_SIZE=50,FONT_SIZE=10;
    if(argv==3){
    ...
    while (1)
    {
        ...
        if (buffer.EventType == KEY_EVENT && buffer.Event.KeyEvent.bKeyDown && (buffer.Event.KeyEvent.uChar.AsciiChar == ' ' || buffer.Event.KeyEvent.uChar.AsciiChar == 'z'))
        {
            if (buffer.Event.KeyEvent.uChar.AsciiChar == ' ')
            {
                ...
                fout = fopen(title, "w");
                for(t1=0;t1<PICTURE_SIZE*PICTURE_SIZE;t1+=PICTURE_SIZE,fprintf(fout,"\n"))
                    fwrite(pic+t1,1,PICTURE_SIZE,fout);
                fclose(fout);
        ...
        else if (buffer.EventType == MOUSE_EVENT && buffer.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)
        {
            ...
            pic[pos.Y       * PICTURE_SIZE + pos.X    ] =
            pic[pos.Y       * PICTURE_SIZE + pos.X - 1] =
            pic[pos.Y       * PICTURE_SIZE + pos.X + 1] =
            pic[(pos.Y - 1) * PICTURE_SIZE + pos.X    ] =
            pic[(pos.Y - 1) * PICTURE_SIZE + pos.X - 1] =
            pic[(pos.Y - 1) * PICTURE_SIZE + pos.X + 1] =
            pic[(pos.Y + 1) * PICTURE_SIZE + pos.X    ] =
            pic[(pos.Y + 1) * PICTURE_SIZE + pos.X - 1] =
            pic[(pos.Y + 1) * PICTURE_SIZE + pos.X + 1] = '1';
            FillConsoleOutputCharacter(hout, '@', 3, {pos.X - 1, pos.Y    }, &t1);
            FillConsoleOutputCharacter(hout, '@', 3, {pos.X - 1, pos.Y - 1}, &t1);
            FillConsoleOutputCharacter(hout, '@', 3, {pos.X - 1, pos.Y + 1}, &t1);
        }
    }
}